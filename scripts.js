$(document).ready(function()  {
	...
	if (location[1] == 'profile') {
        ...
        if (current_user != location[2]) {
	        socket.emit( 'profile check', {
	            ...
	            message: "<a href='profile/"+current_user+"'>"+current_user+"</a> checked your profile"
	        })
	    }
	}
		...
		if(clicked){
			...
			.done(function() {
		        ...
		        socket.emit( 'like event', {
		        	...
		        	visitor_id : current_user_id,
		            host_name : location[2],
		            notification : "<a href='profile/"+current_user+"'>"+current_user+"</a> liked your page"
		        })
				...
				.done(function(data) {
					...
					if (data.value == 'True') {
				        ...
				        socket.emit( 'connection event', {
				        	...
				        	visitor_id : current_user_id,
				            host_name : location[2],
				            notification : "you got a new connection with <a href='profile/"+current_user+"'>"+current_user+"</a>" 
				        })
					}
					else {
						$('#start-chat-button').remove();
			...
			.done(function() {
		        ...
		        socket.emit( 'dislike event', {
		        	...
		        	visitor_id : current_user_id,
		            host_name : location[2],
		            notification : "<a href='profile/"+current_user+"'>"+current_user+"</a> disliked your page :("
		        })
			});
	        clicked = true;
	    }
    ...
    var form = $( '#message_form' ).on( 'submit', function( e ) {
        ...
        socket.emit( 'my event', {
            ...
            message : user_input,
            notification : "you got a new message from <a href='chats/"+current_user+"'>"+current_user+"</a>",
            is_chats : location[1]
        })
        $( '.message' ).val( '' ).focus()
    })
